...
class LobbyCameraTrigger extends hz.Component<typeof LobbyCameraTrigger> {  
  ...
  start() {
    this.connectCodeBlockEvent(this.entity, hz.CodeBlockEvents.OnPlayerEnterTrigger, (player: hz.Player) => {
      console.log("Setting camera lobby with entity", this.props.cameraPositionEntity);
      if (this.props.cameraPositionEntity !== undefined && this.props.cameraPositionEntity !== null) {
        this.sendNetworkEvent(player, PlayerCameraEvents.SetCameraLobby, { entity: this.props.cameraPositionEntity, duration: 0.4, easing: Easing.EaseInOut});
      } else {
