 ...
 */
const LobbyPageViewEvent = new NetworkEvent<{greeting: string}>("LobbyPageViewEvent");
...
class LobbyPageView extends Component<typeof LobbyPageView> {
  ...
  private startServer() {
    ...
    this.connectCodeBlockEvent(this.entity, CodeBlockEvents.OnPlayerEnterWorld, (player: Player) => {
      ...
      this.sendNetworkEvent(player, LobbyPageViewEvent, {greeting: `Welcome ${player.name.get()}`});
    });
  }
