 ...
 */
const ResultWindowViewEvent = new NetworkEvent<{greeting: string}>("ResultWindowViewEvent");
 ...
 */
class ResultWindowView extends Component<typeof ResultWindowView> {
  ...
  private startServer() {
    ...
    this.connectCodeBlockEvent(this.entity, CodeBlockEvents.OnPlayerEnterWorld, (player: Player) => {
      ...
      this.sendNetworkEvent(player, ResultWindowViewEvent, {greeting: `Welcome ${player.name.get()}`});
    });
  }
  ...
  private startClient() {
    ...
    this.connectNetworkEvent(this.world.getLocalPlayer(), ResultWindowViewEvent, data => {
...
Component.register(ResultWindowView);
