...
class PlayerCameraManager extends hz.Component<typeof PlayerCameraManager> {
  ...
  preStart(): void {
    this.connectNetworkBroadcastEvent(CameraManagerEvents.OnRegisterPlayerCamera, ({ObjectId, Object}) => {
      if (ObjectId === "PlayerCamera") {
        console.log("[PlayerCameraManager::preStart] Registering camera " + Object.name.get());
  ...
  start() {
    ...
    this.connectCodeBlockEvent(this.entity, hz.CodeBlockEvents.OnPlayerEnterWorld, (player: hz.Player) => {
      console.log("[PlayerCameraManager::start] Player entered world " + player.name.get());
      if (player.name.get() === "Commander Jeff") return;

      if (player.deviceType.get() !== hz.PlayerDeviceType.VR){
        console.log("[PlayerCameraManager::start] Assigning camera to player", player.name.get());
